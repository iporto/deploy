...
| `Couldn't find the latest tag` no semver | Repositório sem nenhuma tag |
| `compare/v0.0.0...main` → **404** | O `fallbackTag` sozinho não basta: ele define a base do cálculo, mas a ação ainda compara contra essa ref — e ela precisa **existir**. O template cria `v0.0.0` no primeiro commit antes de calcular; se o workflow veio de uma versão anterior, atualize-o |
